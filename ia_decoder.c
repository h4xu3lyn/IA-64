...
static int bound_GvMa_decode(ADDR4 eip, IAinstInfoPtr info)	/* 62 */
{
    ...
    len += modrm_decode(eip+1, info, boundIAEx, bound_GvMa_das,
			BoundSrcRegDest);
...
{
    return 1 + modrm_decode(eip+1, info, arplIAEx, arpl_EwGw_das,
			    RegSrcEaDest);
}
...
static int imul_GvEvIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* 69 */
{
    ...
    len += modrm_decode(eip+1, info, imulIAEx, imul_GvEvIv_das, EaSrcRegDest);
...
static int imul_GvEvIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* 6B */
{
    ...
    len += modrm_decode(eip+1, info, imulIAEx, imul_GvEvIb_das, EaSrcRegDest);
    info->destRdFn = info->srcRdFn;
    info->srcRdFn = immIARd;
...
{
    info->opSize = 1;
    return 1 + modrm_decode(eip+1, info, testIAEx, test_ExGx_das,
			    RegSrcEaDest);
}
...
{
    info->opSize = 1;
    return 1 + modrm_decode(eip+1, info, xchgIAEx, xchg_ExGx_das,
			    RegSrcEaDest);
}
...
{
    info->opSize = 1;
    return 1 + modrm_decode(eip+1, info, movIAEx, mov_ExGx_das, RegSrcEaDest);
}
...
{
    info->opSize = 1;
    return 1 + modrm_decode(eip+1, info, movIAEx, mov_GxEx_das, EaSrcRegDest);
}
...
static int mov_EwSw_decode(ADDR4 eip, IAinstInfoPtr info)	/* 8C */
{
    ...
    info->opSize = 2;
    len += modrm_decode(eip+1, info, movIAEx, mov_EwSw_das, NoSrcEaDest);
    info->srcRdFn = segRegIARd;
    return len;
}
...
static int lea_GvM_decode(ADDR4 eip, IAinstInfoPtr info)	/* 8D */
{
    ...
    len += modrm_decode(eip+1, info, leaIAEx, lea_GvM_das, LeaSrcRegDest);
...
static int mov_SwEw_decode(ADDR4 eip, IAinstInfoPtr info)	/* 8E */
{
    ...
    info->opSize = 2;
    len += modrm_decode(eip+1, info, movIAEx, mov_SwEw_das, EaSrcNoDest);
    info->destRdFn = segRegUpdateIARd;
    info->destWrFn = segRegIAWr;
...
static int pop_Ev_decode(ADDR4 eip, IAinstInfoPtr info)		/* 8F */
{
    ...
    len += modrm_decode(eip+1, info, popIAEx, pop_Ev_das, NoSrcEaDest);
    if (StackAddrSize(info->mode) == IA16)
	info->srcRdFn = pop_spIARd;
    else	/* IA32 */
	info->srcRdFn = pop_espIARd;
...
{
    return iAregReg(info, xchgIAEx, xchg_reg_reg_das, EAX_REG, ECX_REG);
}

/* ARGSUSED */
static int xchg_eAXeDX_decode(ADDR4 eip, IAinstInfoPtr info)	/* 92 */
{
    return iAregReg(info, xchgIAEx, xchg_reg_reg_das, EAX_REG, EDX_REG);
}

/* ARGSUSED */
static int xchg_eAXeBX_decode(ADDR4 eip, IAinstInfoPtr info)	/* 93 */
{
    return iAregReg(info, xchgIAEx, xchg_reg_reg_das, EAX_REG, EBX_REG);
}

/* ARGSUSED */
static int xchg_eAXeSP_decode(ADDR4 eip, IAinstInfoPtr info)	/* 94 */
{
    return iAregReg(info, xchgIAEx, xchg_reg_reg_das, EAX_REG, ESP_REG);
}

/* ARGSUSED */
static int xchg_eAXeBP_decode(ADDR4 eip, IAinstInfoPtr info)	/* 95 */
{
    return iAregReg(info, xchgIAEx, xchg_reg_reg_das, EAX_REG, EBP_REG);
}

/* ARGSUSED */
static int xchg_eAXeSI_decode(ADDR4 eip, IAinstInfoPtr info)	/* 96 */
{
    return iAregReg(info, xchgIAEx, xchg_reg_reg_das, EAX_REG, ESI_REG);
}

static int xchg_eAXeDI_decode(ADDR4 eip, IAinstInfoPtr info)	/* 97 */
{
    return iAregReg(info, xchgIAEx, xchg_reg_reg_das, EAX_REG, EDI_REG);
}
...
{
    info->opSize = 1;
    return iAregImm(eip, info, testIAEx, test_reg_imm_das, AL_REG);
}

static int test_eAXIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* A9 */
{
    return iAregImm(eip, info, testIAEx, test_reg_imm_das, EAX_REG);
}
...
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, AL_REG);
}

static int mov_CLIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* B1 */
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, CL_REG);
}

static int mov_DLIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* B2 */
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, DL_REG);
}

static int mov_BLIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* B3 */
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, BL_REG);
}

static int mov_AHIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* B4 */
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, AH_REG);
}

static int mov_CHIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* B5 */
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, CH_REG);
}

static int mov_DHIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* B6 */
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, DH_REG);
}

static int mov_BHIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* B7 */
{
    info->opSize = 1;
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, BH_REG);
}

static int mov_eAXIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* B8 */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, EAX_REG);
}

static int mov_eCXIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* B9 */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, ECX_REG);
}

static int mov_eDXIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* BA */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, EDX_REG);
}

static int mov_eBXIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* BB */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, EBX_REG);
}

static int mov_eSPIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* BC */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, ESP_REG);
}

static int mov_eBPIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* BD */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, EBP_REG);
}

static int mov_eSIIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* BE */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, ESI_REG);
}

static int mov_eDIIv_decode(ADDR4 eip, IAinstInfoPtr info)	/* BF */
{
    return iAregImm(eip, info, movIAEx, mov_reg_imm_das, EDI_REG);
}
...
static int les_GvMp_decode(ADDR4 eip, IAinstInfoPtr info)	/* C4 */
{
    ...
    len += modrm_decode(eip+1, info, load_far_ptrIAEx, load_far_ptr_das,
			LfpSrcRegDest);
...
static int mov_EbIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* C6 */
{
    ...
    info->opSize = 1;
    len += modrm_decode(eip+1, info, movIAEx, mov_ExIx_das, NoSrcEaDest);
    len += iAimm(eip+len, &info->imm32, 1);
    info->srcRdFn = immIARd;
    return len;
}
...
{
    return 1 + modrm_decode(eip+1, info, larIAEx, lar_GvEw_das, EaSrcRegDest);
}

static int lsl_GvEw_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,03 */
{
    return 1 + modrm_decode(eip+1, info, lslIAEx, lsl_GvEw_das, EaSrcRegDest);
}
...
static int mov_RdCd_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,20 */
{
    ...
    info->opSize = 4;
    len += modrm_decode(eip+1, info, movCRIAEx, mov_RdCd_das, NoSrcEaDest);
    /* XXX - add CR srcRdFn */
...
static int mov_RdDd_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,21 */
{
    ...
    info->opSize = 4;
    len += modrm_decode(eip+1, info, movDRIAEx, mov_RdDd_das, NoSrcEaDest);
    /* XXX - add DR srcRdFn */
...
static int mov_CdRd_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,22 */
{
    ...
    info->opSize = 4;
    len += modrm_decode(eip+1, info, movCRIAEx, mov_CdRd_das, EaSrcNoDest);
...
static int mov_DdRd_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,23 */
{
    ...
    info->opSize = 4;
    len += modrm_decode(eip+1, info, movDRIAEx, mov_DdRd_das, EaSrcNoDest);
...
{
    info->opSize = 4;
    return 1 + modrm_decode(eip+1, info, reservedIAEx, mov_RdTd_das,
			    NoSrcEaDest);
}
...
{
    info->opSize = 4;
    return 1 + modrm_decode(eip+1, info, reservedIAEx, mov_TdRd_das,
			    EaSrcNoDest);
}
...
{
    info->cond_seg = JO_COND;
    return 1 + modrm_decode(eip+1, info, cmovccIAEx, cmovcc_das, EaSrcRegDest);
}
...
static int seto_Eb_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,90 */
{
    info->opSize = 1;
    info->cond_seg = JO_COND;
    return 1 + modrm_decode(eip+1, info, setccIAEx, setcc_das, NoSrcEaDest);
}
...
{
    return 1 + modrm_decode(eip+1, info, btIAEx, bt_EvGv_das, RegSrcEaDest);
}
...
static int shld_EvGvIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,A4 */
{
    ...
    len += modrm_decode(eip+1, info, shld_immIAEx, shld_EvGvIb_das,
			RegSrcEaDest);
    len += iAimm(eip+len, &info->imm32, 1);
    return len;
}
...
{
    return 1 + modrm_decode(eip+1, info, shld_CLIAEx, shld_EvGvCL_das,
			    RegSrcEaDest);
}
...
{
    return 1 + modrm_decode(eip+1, info, btsIAEx, bts_EvGv_das, RegSrcEaDest);
}
...
static int shrd_EvGvIb_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,AC */
{
    ...
    len += modrm_decode(eip+1, info, shrd_immIAEx, shrd_EvGvIb_das,
			RegSrcEaDest);
    len += iAimm(eip+len, &info->imm32, 1);
    return len;
}
...
{
    return 1 + modrm_decode(eip+1, info, shrd_CLIAEx, shrd_EvGvCL_das,
			    RegSrcEaDest);
}
...
{
    return 1 + modrm_decode(eip+1, info, imulIAEx, imul_GvEv_das,
			    EaSrcRegDest);
}
...
{
    info->opSize = 1;
    return 1 + modrm_decode(eip+1, info, cmpxchgIAEx, cmpxchg_ExGx_das,
			    RegSrcEaDest);
}
...
{
    return 1 + modrm_decode(eip+1, info, btrIAEx, btr_EvGv_das, RegSrcEaDest);
}
...
static int movzx_GvEb_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,B6 */
{
    ...
    info->opSize = 1;
    len += modrm_decode(eip+1, info, movzx8IAEx, movzx_GvEb_das,
			EaSrcNoDest);
...
static int movzx_GvEw_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,B7 */
{
    ...
    info->opSize = 2;
    len += modrm_decode(eip+1, info, movzx16IAEx, movzx_GvEw_das,
			EaSrcNoDest);
...
{
    return 1 + modrm_decode(eip+1, info, btcIAEx, btc_EvGv_das, RegSrcEaDest);
}

static int bsf_GvEv_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,BC */
{
    return 1 + modrm_decode(eip+1, info, bsfIAEx, bsf_GvEv_das,
			    EaSrcRegDest);
}

static int bsr_GvEv_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,BD */
{
    return 1 + modrm_decode(eip+1, info, bsrIAEx, bsr_GvEv_das,
			    EaSrcRegDest);
}
...
static int movsx_GvEb_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,BE */
{
    ...
    /* force opSize to 1 to get correct srcRdFn */
    info->opSize = 1;
    len += modrm_decode(eip+1, info, movsx8IAEx, movsx_GvEb_das,
			EaSrcNoDest);
    if ((info->opSize = opSize) == IA16)	/* restore */
	info->destWrFn = reg16IAWr;
    else
...
static int movsx_GvEw_decode(ADDR4 eip, IAinstInfoPtr info)	/* 0F,BF */
{
    ...
    /* force opSize to 2 to get correct srcRdFn */
    info->opSize = 2;
    len += modrm_decode(eip+1, info, movsx16IAEx, movsx_GvEw_das,
			EaSrcNoDest);
    info->opSize = opSize;			/* restore */
    info->destWrFn = reg32IAWr;
    return len;
...
{
    info->opSize = 1;
    return 1 + modrm_decode(eip+1, info, xaddIAEx, xadd_ExGx_das,
			    RegSrcEaDest);
}
