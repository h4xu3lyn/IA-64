...
int brCnt(ADDR a)
{
	...
	return 0;
    t = bundle_decode(b, instr, 0);
    return (t->slot[0].unit == B_Unit) +
	   (t->slot[1].unit == B_Unit) +
	   (t->slot[2].unit == B_Unit);
}
...
BRTYPE brType(ADDR a)
{
    ...
    pdecFn = instrs[instr[s].instID].pdecFn;
    /* chk.s.i */
    if (t->slot[s].unit == I_Unit && pdecFn == &I20predecode)
	return BRT_IPRELJUMP;
    /* chk.s.m, chk.a */
    if (t->slot[s].unit == M_Unit &&
	(pdecFn == &M20predecode || pdecFn == &M21predecode ||
	 pdecFn == &M22predecode || pdecFn == &M23predecode))
	return BRT_IPRELJUMP;
    /* fchkf */
    if (t->slot[s].unit == F_Unit && pdecFn == &F14predecode)
	return BRT_IPRELJUMP;
    if (t->slot[s].unit != B_Unit)
	return BRT_NONE;
    if (pdecFn == &B1predecode || pdecFn == &B2predecode)
	return BRT_IPRELJUMP;
    if (pdecFn == &B3predecode)
	return BRT_IPRELCALL;
    if (pdecFn == &B4predecode)
	return instrs[instr[s].instID].combFn == &br_ret_spnt_few_b2Comb ?
		BRT_RET : BRT_INDJUMP;
    if (pdecFn == &B5predecode)
	return BRT_INDCALL;
    return BRT_NONE;
}
...
void brTargets(ADDR a, ADDR *taken, ADDR *fallthru)
{
    ...
    *fallthru = nextIp(a);
    ...
    pdecFn = instrs[instr[s].instID].pdecFn;
    pdecFn(instr[s].instrBits, &info);
    *taken = BADDR(a) + info.immed64;
}

BOOL brConditional(ADDR a)
{
    BundlePtr b;
    DecodedInstr instr[SLOTS_PER_BUNDLE];
    unsigned s;
...
BOOL brTaken(ADDR a, ADDR *next)
{
    ...
    pdecFn(instr[s].instrBits, &info);
    /* chk.s r2,target25 */
    if (pdecFn == &I20predecode || pdecFn == &M20predecode) {
	REG junk;
	BOOL nat;
	...
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
    ...
    /* chk.s f2,target25 */
    if (pdecFn == &M21predecode) {
	FREG fr;

	fr = FrRd(info.extrainfo[2]);
	/* XXX - include float.h for CLASS_NAT? */
	tkn = PrRd(info.qpred) && fr.special && fr.class == 3/*CLASS_NAT*/;
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
	return tkn;
    }
    /* chk.a r1,target25 */
    if (pdecFn == &M22predecode) {
	tkn = PrRd(info.qpred) &&
		(!use_alat || alat_cmp(NO, info.extrainfo[1], NO));
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
	return tkn;
    }
    /* chk.a f1,target25 */
    if (pdecFn == &M23predecode) {
	tkn = PrRd(info.qpred) &&
		(!use_alat || alat_cmp(YES, info.extrainfo[1], NO));
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
	return tkn;
    }
    /* fchkf target25 */
    if (pdecFn == &F14predecode) {
	switch (instr[s].instID) {
	    case EM_FCHKF_S0_TARGET25:
		tkn = PrRd(info.qpred) &&
		      ((FPSR_SF0_FLAGS & ~FPSR_TRAPS) ||
		       (FPSR_SF0_FLAGS & ~FPSR_SF0_FLAGS));
		break;
	    case EM_FCHKF_S1_TARGET25:
		tkn = PrRd(info.qpred) &&
		      ((FPSR_SF1_FLAGS & ~FPSR_TRAPS) ||
		       (FPSR_SF1_FLAGS & ~FPSR_SF0_FLAGS));
		break;
	    case EM_FCHKF_S2_TARGET25:
		tkn = PrRd(info.qpred) &&
		      ((FPSR_SF2_FLAGS & ~FPSR_TRAPS) ||
		       (FPSR_SF2_FLAGS & ~FPSR_SF0_FLAGS));
		break;
	    case EM_FCHKF_S3_TARGET25:
		tkn = PrRd(info.qpred) &&
		      ((FPSR_SF3_FLAGS & ~FPSR_TRAPS) ||
	...
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
    ...
    if (pdecFn == &B4predecode || pdecFn == &B5predecode) {
	tkn = PrRd(info.qpred);
	*next = tkn ? BrRd(info.extrainfo[2]) : nextIp(a);
	return tkn;
    }
    if (pdecFn == &B3predecode) {
	tkn = PrRd(info.qpred);
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
    ...
    if (pdecFn == &B2predecode) {
	if (instrs[instr[s].instID].combFn == &br_cloop_spnt_few_target25Comb)
	    tkn = LC != 0;
	if (instrs[instr[s].instID].combFn == &br_ctop_spnt_few_target25Comb)
	    tkn = LC != 0 || EC > 1;
	if (instrs[instr[s].instID].combFn == &br_cexit_spnt_few_target25Comb)
	    tkn = LC == 0 && EC <= 1;
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
	return tkn;
    }
    if (pdecFn == &B1predecode) {
	if (instrs[instr[s].instID].combFn == &br_wtop_spnt_few_target25Comb)
	    tkn = PrRd(info.qpred) || EC > 1;
	if (instrs[instr[s].instID].combFn == &br_wexit_spnt_few_target25Comb)
	    tkn = !PrRd(info.qpred) && EC <= 1;
	if (instrs[instr[s].instID].combFn == &br_cond_spnt_few_target25Comb)
	    tkn = PrRd(info.qpred);
	*next = tkn ? BADDR(a) + info.immed64 : nextIp(a);
	return tkn;
    }
